...
describe('SolicitudController', () => {
  ...
  beforeEach(async () => {
    ...
    const SolicitudServiceProvider = {provide: SolicitudService,useClass: SolicitudServiceMock}
  ...
  it('should update a solicitud', async () => {
    ...
    expect(await controller.update(solicitudId.toString(), updateSolicitudDto)).toEqual({id: solicitudId,...updateSolicitudDto});
  ...
  it('should find a solicitud', async () => { 
    const solicitudId = 2;
    expect((await controller.findOne(solicitudId.toString())).id).toEqual(
      solicitudId,
    );
  });
